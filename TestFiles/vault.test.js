...
contract("Vault", (accounts) => {
  ...
  describe ("Use Case 1: user deposits ether and receives stablecoin ", async ()=>{
    ...
    it("should update user Vault collateral with sent Ether", async ()=>{
      ...
      assert.equal(toWei("1"), userVault.collateralAmount, "user collateral and sent ether do not match");
    })
    ...
    it ("should update user token balance", async ()=>{
      ...
      assert.equal(updatedTokenBalance.toString(), expectedTokenBalance.toString(), "the user Token balance has not been updated with the right amount")
    })

    it ("should update user Vault debt", async ()=>{
      let userVault = await vault.getVault(user);
      assert.equal(updatedTokenBalance.toString(), userVault.debtAmount.toString(), "The right amount has not been updated by the user Vault debt")
    })
  ...
  describe ("Use Case 2: user repays ALL tokens and withdraws ether ", async ()=>{
    ...
    it ("user token balance should be zero", async ()=>{
      assert.equal(finalTokenBalance.toString(), "0", "user Token balance will be zero after the repayment")
    })

    it ("user vault debt should be zero", async ()=>{
      let userVault = await vault.getVault(user);
      assert.equal("0", userVault.debtAmount.toString(), "user Vault debt is not zero after the repayment")
    })
